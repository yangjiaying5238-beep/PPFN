...
mode = 'Restormer-PPFN_' + args.dataset_name +'_deg-L1_SSIM'
...
ssim_loss = SSIM(data_range=1.0, channel=1).cuda()
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(tbar):
        ...
        for idx in range(3):
            ...
            # Compute loss
            loss_l1 = L1loss(restored, tar)
            loss_ssim = 1 - ssim_loss(restored, tar)

            loss_each = loss_l1 + 0.1 * loss_ssim
            if idx < 2:
