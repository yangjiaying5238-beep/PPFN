...
class F_ext(nn.Module):
    # this code is taken from https://github.com/Xiangtaokong/MiOIR
...
class T_ext(nn.Module):
    def __init__(self, nf=64):
        super(T_ext, self).__init__()
        self.layer1 = nn.Linear(nf, nf)
        self.layer2 = nn.Linear(nf, nf)
        self.layer3 = nn.Linear(nf, nf)
        self.act = nn.ReLU(inplace=True)

    def forward(self, x):
        layer1_out = self.act(self.layer1(x))
        layer2_out = self.act(self.layer2(layer1_out))
        layer3_out = self.act(self.layer3(layer2_out))
...
class Ext(nn.Module):
    def __init__(self, in_nc=3, nf=64):
        ...
        self.F_ext = F_ext(in_nc, nf)
        self.T_ext = T_ext(nf)
        self.layerF = nn.Linear(nf, nf)
        self.layerT = nn.Linear(nf, nf)
        self.act = nn.ReLU(inplace=True)

    def forward(self, x, y):
        layerF_out = self.F_ext(x)
        layerT_out = self.T_ext(y)
        out = self.act(self.layerF(layerF_out)+self.layerT(layerT_out))

        return out

class Restormer_PPFN(nn.Module):
    ...
    def __init__(self,
        ...
        super(Restormer_PPFN, self).__init__()

        prompt_nf = 64

        self.F_ext_net1 = Ext(in_nc=1, nf=64)
        self.F_ext_net2 = Ext(in_nc=1, nf=64)
        self.F_ext_net3 = Ext(in_nc=1, nf=64)
        self.F_ext_net4 = Ext(in_nc=1, nf=64)
        self.dim=dim
    ...
    def forward(self, inp_img):
        ...
        tprompt = torch.unsqueeze(inp_img[1], 1)
        mprompt = inp_img[2]

        prompt1 = self.F_ext_net1(tprompt, mprompt)
        prompt2 = self.F_ext_net2(tprompt, mprompt)
        prompt3 = self.F_ext_net3(tprompt, mprompt)
        prompt4 = self.F_ext_net4(tprompt, mprompt)
